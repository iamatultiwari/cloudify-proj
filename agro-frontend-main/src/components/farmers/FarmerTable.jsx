...
const FarmerTable = ({ farmers, deleteFarmer }) => {
  return (
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50 border-b border-slate-100">
              ...
              <th className="px-8 py-5 text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">Region & Contact</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {farmers?.length === 0 ? (
              ...
              farmers?.map((farmer) => (
                <tr key={farmer._id} className="hover:bg-slate-50/50 transition-colors group">
                  {/* Identity Column */}
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 font-black text-lg border-2 border-emerald-100/50">
                  ...
                  {/* Region & Contact Column (Updated with District) */}
                  <td className="px-8 py-6">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-black text-slate-600 uppercase tracking-wider">
                        ...
                        {farmer.village || "Unknown"}, {farmer.district || "Unknown"}
                      </div>
                    </div>
                  </td>

                  {/* Financial Status Column (Fixed with Safety Check) */}
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className={`text-lg font-black tracking-tight flex items-center gap-1 ${(farmer.dueAmount || 0) > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                        <IndianRupee size={16} />
                        {(farmer.dueAmount || 0).toLocaleString()}
                      </span>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                      <Link to={`/farmers/${farmer._id}`} className="p-2.5 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                        <Eye size={18} />
                      </Link>
                      <Link to={`/farmers/edit/${farmer._id}`} className="p-2.5 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-600 hover:text-white transition-all shadow-sm">
                        <Edit3 size={18} />
                      </Link>
                      <button onClick={() => deleteFarmer(farmer._id)} className="p-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
