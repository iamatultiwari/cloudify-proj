...
export const salesReport = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      overallSales: overallSales[0] || { totalRevenue: 0, totalGST: 0, totalInvoices: 0 },
    });
  } catch (error) {
...
export const farmerDueReport = async (req, res) => {
  try {
    const farmers = await Farmer.find({ dueAmount: { $gt: 0 } })
      .sort({ dueAmount: -1 });
    ...
    res.status(200).json({
      success: true,
      totalPendingDue: totalDue[0]?.totalPendingDue || 0,
      farmers,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
export const stockReport = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      totalStockValue: totalStockValue[0]?.stockValue || 0,
      products,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
export const dashboardSummary = async (req, res) => {
  try {
    ...
    const pendingPayments = await Farmer.aggregate([
    ...
    const overdueBills = await Transaction.find({
      type: "credit",
      dueDate: { $lte: today },
    });
    ...
    const interestEarned = await Transaction.aggregate([
      { $match: { type: "interest" } },
      { $group: { _id: null, totalInterest: { $sum: "$amount" } } },
    ]);
    ...
    res.status(200).json({
      ...
      dashboard: {
        ...
        pendingPayments: pendingPayments[0]?.totalDue || 0,
